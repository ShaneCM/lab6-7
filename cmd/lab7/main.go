...
func main() {
	...
	router.GET("/query1", func(c *gin.Context) {
		...
		// put your query here
		rows, err := db.Query("SELECT name FROM people WHERE city = 'seattle'") // <--- EDIT THIS LINE
		if err != nil {
	...
	router.GET("/query2", func(c *gin.Context) {
		table := "<table class='table'><thead><tr>"
		// put your query here
		rows, err := db.Query("SELECT name FROM superheroes WHERE name IN (SELECT name FROM people WHERE city = 'seattle' AND street = 'state st')") // <--- EDIT THIS LINE
		if err != nil {
			// careful about returning errors to the user!
			c.AbortWithError(http.StatusInternalServerError, err)
		}
	...
	router.GET("/query3", func(c *gin.Context) {
		table := "<table class='table'><thead><tr>"
		// put your query here
		rows, err := db.Query("SELECT COUNT(*) AS total FROM superheroes JOIN people ON superheroes.name = people.name WHERE city IN (SELECT city FROM cities WHERE state = 'wa')") // <--- EDIT THIS LINE
		if err != nil {
			// careful about returning errors to the user!
			c.AbortWithError(http.StatusInternalServerError, err)
		}
		...
		var total int;
		for rows.Next() {
			rows.Scan(&total) // put columns here prefaced with &
